...
describe.only("Test suite for Aave", () => {
    before(async () => {
        ...
        debtGHO = await ethers.getContractAt("MockERC20", "0x786dbff3f1292ae8f92ea68cf93c30b34b1ed04b")
        aToken = await ethers.getContractAt("MockERC20", "0x98C23E9d8f34FEFb1B7BD6a91B7FF122F4e16F5c")
        ...
        await setStorageAt(
            USDC,
            ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(["address", "uint256"], [manager.address, 9])),
    ...
    it("Test suite", async () => {
        ...
        console.log("User vault balance: ", (await vault.balanceOf(manager.address)).toString());
        console.log("Vault USDC balance: ", (await usdc.balanceOf(vault.address)).toString());

        await vault.supplyCollateral(usdcDepositAmount.div(bn(2)));
        const ghoMintAmount = ((usdcDepositAmount.div(bn(2)).mul(bn(10).pow(12))).mul(70).div(100));
        await vault.mintGHO(ghoMintAmount);
        console.log("After supplying 50k usdc to aave as supply and borrowing 35k GHO");
        console.log("Vault USDC balance: ", (await usdc.balanceOf(vault.address)).toString());
        console.log("Vault GHO balance: ", (await gho.balanceOf(vault.address)).toString());
        ...
        const data = await (await vault.addLiquidity(lowerTick, upperTick, ghoBalance, usdcBalance)).wait();
        console.log("Vault balance after adding maximum liquidity to uniswap v3 0.3% pool");
        console.log("Vault USDC balance: ", (await usdc.balanceOf(vault.address)).toString());
        console.log("Vault GHO balance: ", (await gho.balanceOf(vault.address)).toString());

        await vault.swap(false, ethers.utils.parseUnits("1000", 6), bn("146144670348521010328727305220398882237872397034"));
        ...
        let {totalCollateralBase, totalDebtBase} = await vault.getAavePositionData();
        ...
        console.log("Vault USDC balance: ", (await usdc.balanceOf(vault.address)).toString());
        console.log("Vault GHO balance: ", (await gho.balanceOf(vault.address)).toString());
        ({totalCollateralBase, totalDebtBase} = await vault.getAavePositionData());
        ...
        console.log((await vault.getUnderlyingBalance()).toString(), (await vault.balanceOf(manager.address)).toString(), (await vault.totalSupply()).toString())
