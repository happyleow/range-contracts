...
import {IElixirPool} from "./elixir/interfaces/IElixirPool.sol";

import {TickMath} from "./elixir/TickMath.sol";
import {LiquidityAmounts} from "./elixir/LiquidityAmounts.sol";
import {FullMath} from "./elixir/FullMath.sol";
 ...
 * @notice RangeProtocolVault is fungible vault shares contract that accepts elixir pool tokens for liquidity
 * provision to the corresponding elixir pool. This contract is configurable to work with any elixir
 ...
 * active position in the elixir pool) and burning involves removing liquidity from the elixir pool along with
 * the vault's fee.
 *
 * The manager of the contract can remove liquidity from elixir pool and deposit into a newer take range to maximise
 * the profit by keeping liquidity out of the pool under high volatility periods.
 *
 * Part of the fee earned from elixir position is paid to manager as performance fee and fee is charged on the LP's
    ...
    RangeProtocolVaultStorage
{
    ...
    /// Performance fee cannot be set more than 10% of the fee earned from elixir pool.
     ...
     * @param _pool address of the elixir pool associated with vault
     * @param _tickSpacing tick spacing of the elixir pool
    ...
    /// @notice elixirMintCallback Elixir  callback fn, called back on pool.mint
    ...
    /// @notice elixirSwapCallback Elixir callback fn, called back on pool.swap
    ...
    /**
     * @notice mint mints range vault shares, fractional shares of a Elixir  position/strategy
    ...
    /**
     * @notice burn burns range vault shares (shares of a Elixir  position) and receive underlying
    ...
    /**
     * @notice removeLiquidity removes liquidity from elixir pool and receives underlying tokens
    ...
    /**
     * @dev Mars@RangeProtocol
     * @notice addLiquidity allows manager to add liquidity into elixir pool into newer tick ranges.
    ...
    /**
     * @dev pullFeeFromPool pulls accrued fee from elixir pool that position has accrued since
     * last collection.
     */
     ...
     * includes current liquidity invested in elixir position, current fees earned
    ...
    /**
     * @notice getPositionID returns the position id of the vault in elixir pool
     * @return positionID position id of the vault in elixir pool
     */
    ...
    /**
     * @notice _withdraw internal function to withdraw liquidity from elixir pool
     * @param liquidity liquidity to remove from the elixir pool
     */
    function _withdraw(
        uint128 liquidity
     ...
     * @param feeGrowthInsideLast last time the fee was realized for the vault in elixir pool
     */
    ...
    /**
     * @notice _applyPerformanceFee applies the performance fee to the fees earned from elixir pool.
    ...
    /**
     * @notice _netPerformanceFees computes the fee share for manager as performance fee from the fee earned from elixir pool.
     * @param rawFee0 fee earned in token0 from elixir pool.
     * @param rawFee1 fee earned in token1 from elixir pool.
